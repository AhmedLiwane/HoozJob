...
const ResetPasswordV1 = () => {
  ...
  // Handle New Password
  const handleNewPasswordChange = prop => async event => {
  ...
  const handleConfirmNewPasswordChange = prop => async event => {
    setValues({ ...values, [prop]: event.target.value })
  ...
  const onSubmit = data => {
    const password = values.confirmNewPassword
    auth.confirmResetPassword({ token, password }, () => {
      setError('password', {
        type: 'manual',
