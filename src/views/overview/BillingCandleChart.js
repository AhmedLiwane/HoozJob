...
const BillingCandleChart = ({ ComapanyState }) => {
  // ** States
  const [endDate, setEndDate] = useState(null)
  const [startDate, setStartDate] = useState(null)
  const [lastAmount, setLastAmount] = useState([])
  const [currentAm, setCurrentAm] = useState([])
  ...
  const filter = async () => {
    ...
    ComapanyState?.billing?.spendings?.map(spend => {
      const date = moment(spend.creationDate.toString()).format('MM-DD-YYYY')
  ...
  const series = [
    {
      name: 'Male',
      data: lastAmount ? lastAmount : [0, 0, 0, 0, 0, 0, 0]
    },
    {
      name: 'Not Selected',
      data: currentAm ? currentAm : [0, 0, 0, 0, 0, 0, 0]
    }
  ]
...
export default BillingCandleChart
